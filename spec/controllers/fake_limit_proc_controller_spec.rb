...
RSpec.describe FakeLimitProcController, type: :controller do
  before do
  ...
  describe '.rate_limit' do
    ...
    context 'with block and lambda for limit' do
      ...
      before do
        allow(FakeLimitProcController).to receive(:before_action)
        allow(RailsRateLimiter::Strategies::SlidingWindowLog).to receive(:new)
          .with(99, 56.minutes, 'custom_current-user-id', nil)
          .and_return(strategy)
        ...
        FakeLimitProcController.class_eval do
      ...
      context 'when limit exceeded' do
        ...
        it 'runs required operations' do
          expect(FakeLimitProcController).to have_received(:before_action)
